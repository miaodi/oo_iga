...
int main()
{
    ...
    for ( int d = 4; d < degree; ++d )
    {
        for ( int r = 1; r < refine; ++r )
        {
            ...
            // constraint_assemble.AssembleConstraintWithAdditionalConstraint( constraint );

            // vector<pair<vector<int>, MatrixXd>> storage;
            // for ( auto& i : cells )
            // {
            //     int starting_dof = dof.StartingDof( i->GetID() );
            //     auto domain = i->GetDomain();
            //     Vector2d para;
            //     if ( domain->InversePts( point5, para ) )
            //     {
            //         auto evals = domain->Eval2PhyDerAllTensor( para );
            //         vector<int> index;
            //         for ( auto ind : *evals )
            //         {
            //             index.push_back( ind.first + starting_dof );
            //         }
            //         MatrixXd temp( 3, index.size() );
            //         temp.setZero();
            //         for ( int m = 0; m < 3; m++ )
            //         {
            //             for ( int n = 0; n < index.size(); n++ )
            //             {
            //                 if ( abs( ( *evals )[n].second[m] ) > 1e-13 )
            //                 {
            //                     temp( m, n ) = ( *evals )[n].second[m];
            //                 }
            //             }
            //         }
            //         // for ( int n = 0; n < index.size(); n++ )
            //         // {
            //         //     temp( 3, n ) = ( *evals )[n].second[4];
            //         // }

            //         storage.push_back( make_pair( index, temp ) );
            //     }
            // }
            ...
            //     for ( int j = 0; j < storage[0].first.size(); j++ )
            //     {
            //         sparse_vector.coeffRef( storage[0].first[j] ) = storage[0].second( i, j );
            //     }
            //     for ( int j = 0; j < storage[1].first.size(); j++ )
            //     {
            //         sparse_vector.coeffRef( storage[1].first[j] ) = -storage[1].second( i, j );
            //     }
            ...
            //         sparse_vector.coeffRef( storage[0].first[j] ) = storage[0].second( i, j );
            //     }
            //     for ( int j = 0; j < storage[2].first.size(); j++ )
            //     {
            //         sparse_vector.coeffRef( storage[2].first[j] ) = -storage[2].second( i, j );
            //     }
            //     constraint.bottomRows( 1 ) = sparse_vector.transpose();
            // }
            // MatrixXd dense_constraint = MatrixXd( constraint );
            // JacobiSVD<MatrixXd> svd( dense_constraint, ComputeThinU | ComputeFullV );
            ...
            //     count++;
            // }
            // SparseMatrix<double> sp = svd.matrixV().rightCols( svd.matrixV().cols() - count ).sparseView( 1e-15 );
            ...
            // cout << constraint * sp1 << endl;
            // cout << constraint.rows() << " " << constraint.cols() << endl;
            // cout << sp1.rows() << " " << sp1.cols() << endl;
