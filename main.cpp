...
int main() {
    ...
    domain4->DegreeElevate(3);
    domain5->DegreeElevate(3);
    ...
    cells[0]->PrintEdgeInfo();
    cells[1]->PrintEdgeInfo();
    DofMapper<double> s;
    ...
    PoissonVisitor<double> poisson(s, [&pi](Coordinate u) -> vector<double> {
        return vector<double>{2 * pow(pi, 2) * sin(pi * u(0)) * sin(pi * u(1))};
    });
    ...
    unique_ptr<SparseMatrix<double>> stiffnessInterface;
    stiffnessInterface = interface.DGInterface();
    function<vector<double>(const Coordinate &)> Analytical = [&pi](const Coordinate &u) {
        return vector<double>{sin(pi * u(0)) * sin(pi * u(1))};
    };
    PoissonDGBoundaryVisitor<double> boundary(s, Analytical);
    ...
    SparseMatrix<double> stiffnessSol = *stiffness + (*boundaryStiffness + *stiffnessInterface);
