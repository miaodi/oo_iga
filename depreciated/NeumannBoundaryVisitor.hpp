...
class NeumannBoundaryVisitor : public DomainVisitor<1, 2, T>
{
  ...
  public:
    ...
    void
    NeumannBoundaryAssembler(Eigen::SparseMatrix<T> &) const;

  protected:
    //    Assemble stiffness matrix and rhs
    void
        LocalAssemble(Element<1, 2, T> *, const QuadratureRule<T> &, const KnotSpan &);

    virtual void
    IntegralElementAssembler(Matrix &linear_form_value, std::vector<int> &, Matrix &linear_form_test, const Edge<2, T> *edge, const Knot &u) const;
...
template <typename T>
void NeumannBoundaryVisitor<T>::LocalAssemble(Element<1, 2, T> *g,
                                              ...
                                              const KnotSpan &knot_span)
{
    ...
    for (int i = 0; i < quadrature_points.size(); ++i)
    {
        ...
        IntegralElementAssembler(linear_form_value[i], linear_form_test_indices, linear_form_test[i], edge, quadrature_points[i].first);
    }
...
template <typename T>
void NeumannBoundaryVisitor<T>::IntegralElementAssembler(
