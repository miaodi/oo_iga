...
{
public:
    using Knot = typename PoissonInterfaceVisitor<3, T>::Knot;
    using Quadrature = typename PoissonInterfaceVisitor<3, T>::Quadrature;
    using QuadList = typename PoissonInterfaceVisitor<3, T>::QuadList;
    using KnotSpan = typename PoissonInterfaceVisitor<3, T>::KnotSpan;
    using KnotSpanlist = typename PoissonInterfaceVisitor<3, T>::KnotSpanlist;
    using LoadFunctor = typename PoissonInterfaceVisitor<3, T>::LoadFunctor;
    using Matrix = typename PoissonInterfaceVisitor<3, T>::Matrix;
    using Vector = typename PoissonInterfaceVisitor<3, T>::Vector;
    using DomainShared_ptr = typename PoissonInterfaceVisitor<3, T>::DomainShared_ptr;
    using ConstraintIntegralElementAssembler = typename PoissonInterfaceVisitor<3, T>::ConstraintIntegralElementAssembler;

private:
    struct SlaveAndMaster
    {
        SlaveAndMaster()
        {
        }
        SlaveAndMaster( const Vector& s, const Vector& m ) : _slaveQuadrature( s ), _masterQuadrature( m )
        {
        }
...
protected:
    void SolveConstraint( Edge<3, T>* edge )
    {
        ...
        auto vertices_indices = edge->VerticesIndices( 3, 1 );
        // iterate across the constraint equation container and obtain activated local indices and lagrange multiplier indices
        std::vector<int> slave_side_activated_indices = Accessory::ColIndicesVector( this->_c0Slave );
        std::vector<int> master_activated_indices = Accessory::ColIndicesVector( this->_c0Master );
        std::vector<int> multiplier_indices = Accessory::RowIndicesVector( this->_c0Slave );
    ...
    void LocalAssemble( Element<1, 3, T>* g, const QuadratureRule<T>& quadrature_rule, const KnotSpan& knot_span )
    {
        ...
        using namespace std::placeholders;
        auto c0_function =
            std::bind( &KLShellC0InterfaceVisitor<T>::C0IntegralElementAssembler, this, _1, _2, _3, _4, _5, _6, _7, _8, _9 );
        this->ConstraintLocalAssemble( g, quadrature_rule, knot_span, c0_function, this->_c0Slave, this->_c0Master );
    }

protected:
    MatrixData<T> _slaveMasterConstraintData;
    std::unordered_map<T, SlaveAndMaster> _quadratureMap;
};