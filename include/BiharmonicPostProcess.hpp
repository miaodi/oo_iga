...
class BiharmonicPostProcess : public DomainVisitor<2, N, T>
{
  public:
  ...
  public:
    BiharmonicPostProcess(const DofMapper<N, T> &dof_mapper, const Vector &solution, const LoadFunctor &analytical_solution) : DomainVisitor<2, N, T>(dof_mapper), _solution(solution), _analyticalSolution(analytical_solution) {}
    T L2Norm() const;
