...
protected:
    ...
    void InitializeQuadratureRule( Element<1, N, T>* g, QuadratureRule<T>& quad_rule );

    // Initialize knot spans such that new knot span is a union of two coupling knot vectors
    void InitializeKnotSpans( Element<1, N, T>* g, KnotSpanlist& knot_spans );

    virtual void SolveConstraint( Edge<N, T>* ) = 0;

    void ConstraintLocalAssemble( Element<1, N, T>*,
...
void InterfaceVisitor<N, T>::Visit( Element<1, N, T>* g )
{
    ...
    {
        // needs to modify BezierDualInitialize();
...
template <int N, typename T>
void InterfaceVisitor<N, T>::ConstraintLocalAssemble( Element<1, N, T>* g,
                                                      ...
                                                      std::vector<Eigen::Triplet<T>>& master_constraints_equation_element )
{
    ...
    {
        IntegralElementAssembler( slave_constraint_basis[i], slave_constraint_basis_indices, master_constraint_basis[i],
                                  master_constraint_basis_indices, multiplier_basis[i], multiplier_basis_indices,
                                  weights[i], edge, edge_quadrature_points[i] );
    }
    auto multiplier_basis_indices_copy{multiplier_basis_indices};
    auto stiff = this->LocalStiffness( multiplier_basis, multiplier_basis_indices, slave_constraint_basis,
                                       slave_constraint_basis_indices, weights );
    auto load = this->LocalStiffness( multiplier_basis, multiplier_basis_indices_copy, master_constraint_basis,
                                      master_constraint_basis_indices, weights );
