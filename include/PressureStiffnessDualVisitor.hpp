...
template <typename T>
class PressureStiffnessDualVisitor : public DomainVisitor<2, 2, T>
{
  public:
...
template <typename T>
void PressureStiffnessDualVisitor<T>::Initialize(Element<2, 2, T> *g)
{
...
template <typename T>
void PressureStiffnessDualVisitor<T>::LocalAssemble(Element<2, 2, T> *g,
                                                ...
                                                const KnotSpan &knot_span)
{
    ...
    std::vector<int> bilinear_form_test_indices{test_index}, bilinear_form_trial_indices{test_index};
...
template <typename T>
void PressureStiffnessDualVisitor<T>::InnerProductAssembler(Eigen::SparseMatrix<T> &sparse_matrix) const
{
...
template <typename T>
void PressureStiffnessDualVisitor<T>::IntegralElementAssembler(
    ...
    const Knot &u) const
{
    ...
    bilinear_form_trail.resize(1, test_evals->size());
