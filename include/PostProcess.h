...
public:
    PostProcess( const std::vector<std::shared_ptr<Surface<2, T>>>& dom, const std::vector<std::shared_ptr<PhyTensorBsplineBasis<2, 1, T>>>& res, const Functor& target )
        : _targetFunction( target )
    {
        ASSERT( dom.size() == res.size(), "The number of computational domains and that of result domains are not consistent." );
            ...
            auto lambda = [&]( typename CoordinatePairList::iterator begin, typename CoordinatePairList::iterator end ) -> void {
                ...
                for ( auto it = begin; it != end; ++it )
                {
                    ...
                    for ( const auto& i : quadratures )
                    {
                        ...
                        err_v += i.second * pow( _targetFunction( result.first->GetDomain()->AffineMap( i.first ) )[0] - result.second->AffineMap( i.first )( 0 ), 2 );
                    }
                }
            ...
            auto lambda = [&]( typename CoordinatePairList::iterator begin, typename CoordinatePairList::iterator end ) -> void {
                ...
                for ( auto it = begin; it != end; ++it )
                {
                    ...
                    for ( const auto& i : quadratures )
                    {
                        ...
                        val_v += i.second *
                                 ( pow( exact[0], 2 ) + pow( exact[1], 2 ) + pow( exact[2], 2 ) + pow( exact[3], 2 ) + pow( exact[4], 2 ) + pow( exact[5], 2 ) );
                        err_v += i.second * ( pow( exact[0] - approx[0], 2 ) + pow( exact[1] - approx[1], 2 ) + pow( exact[2] - approx[2], 2 ) +
                                              pow( exact[3] - approx[3], 2 ) + pow( exact[4] - approx[4], 2 ) + pow( exact[5] - approx[5], 2 ) );
                    }
                }
                std::lock_guard<std::mutex> lock( mtx );
                val_norm_per_patch += val_v;
