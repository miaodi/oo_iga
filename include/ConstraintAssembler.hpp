...
public:
    ...
    void AssembleByReducedKernel( Eigen::SparseMatrix<T>& sparse_kernel_matrix )
    {
        ...
        std::set_intersection( _additional_constraint.begin(), _additional_constraint.end(), _involved_indices.begin(), _involved_indices.end(),
                               std::back_inserter( active_boundary_constraint ) );
        ...
        auto pre_kernel_it =
            std::partition( kernel_vector_container.begin(), kernel_vector_container.end(),
                            [&sparse_constraint_matrix]( const Eigen::SparseVector<T>& ii ) { return ( sparse_constraint_matrix * ii ).norm() > 0; } );
        ...
        // sparse_kernel_matrix = (sparse_pre_kernel_matrix *
        // lu_decomp.kernel()).sparseView(1e-15);
        ...
        std::set_difference( dof_indices.begin(), dof_indices.end(), _involved_indices.begin(), _involved_indices.end(), std::back_inserter( idle_index ) );

        std::vector<int> idle_index_constrained;
        std::set_difference( idle_index.begin(), idle_index.end(), _additional_constraint.begin(), _additional_constraint.end(), std::back_inserter( idle_index_constrained ) );
