...
class BiharmonicDirichletBoundaryVisitor : public DirichletBoundaryVisitor<N, T>
{
    ...
    BiharmonicDirichletBoundaryVisitor( const LoadFunctor& boundary_value ): DirichletBoundaryVisitor<N, T>( boundary_value )
    {
    }
...
template <int N, typename T>
void BiharmonicDirichletBoundaryVisitor<N, T>::IntegralElementAssembler( Matrix& bilinear_form_trail,
                                                                         std::vector<int>& bilinear_form_trail_indices,
                                                                         Matrix& bilinear_form_test,
                                                                         std::vector<int>& bilinear_form_test_indices,
                                                                         Matrix& linear_form_value,
                                                                         ...
                                                                         const Quadrature& u ) const
{
    ...
    for ( int i = 0; i < N; ++i )
    {
        normal_derivative += normal( i ) * this->_dirichletFunctor( trial_domain->AffineMap( trial_quadrature_abscissa ) )[i + 1];
    }
