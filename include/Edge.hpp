...
public:
    ...
    Edge( const Orientation& orient = Orientation::west ) : Element<1, N, T>(), _position( orient ){};

    Edge( DomainShared_ptr m, const Orientation& orient, std::shared_ptr<Vertex<N, T>>& begin, std::shared_ptr<Vertex<N, T>>& end )
    ...
    //
    std::vector<int> VerticesIndices( const int& dimension, const int& layer ) const
    {
        std::vector<int> res;

        auto front_v = _vertices[0]->Indices( dimension, layer );
        auto last_v = _vertices[1]->Indices( dimension, layer );
        std::set_union( front_v.begin(), front_v.end(), last_v.begin(), last_v.end(), std::back_inserter( res ) );
        return res;
    }
    ...
    bool Match( std::shared_ptr<Edge<N, T>> counterpart )
    {
        ...
        if ( ( ( GetStartCoordinate() - counterpart->GetStartCoordinate() ).norm() < tol &&
               ( GetEndCoordinate() - counterpart->GetEndCoordinate() ).norm() < tol ) ||
             ( ( GetStartCoordinate() - counterpart->GetEndCoordinate() ).norm() < tol &&
               ( GetEndCoordinate() - counterpart->GetStartCoordinate() ).norm() < tol ) )
        {
            _pair = counterpart;
            _matched = true;
    ...
    void Accept( Visitor<1, N, T>& visitor )
    {
        visitor.Visit( this );
    };
