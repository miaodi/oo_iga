...
public:
    ...
    SlaveDofIn(const DomainShared_ptr& domain) const
    {
        ...
        ASSERT(it!=_slaveDof.end(), "Given domain does not exist in the _slaveDof list.\n");
    ...
    CondensedIndexMap()
    {
        ...
        for (const auto& i : _domains)
        {
            ...
            {
                int copyIndex = inDomainIndex;
                if (CondensedIndexInDomain(i, copyIndex))
                {
                    res.push_back(startIndex+inDomainIndex);
                }
            }
    ...
    FreeToCondensedIndexMap()
    {
        ...
        {
            auto it = std::find(condensed_indices.begin(), condensed_indices.end(), i);
            ASSERT(it!=condensed_indices.end(), "Inconsistence happens between free indices and condensed indices.");
            res.push_back(*it);
        }
        return res;
    }
    ...
    PrintFreeDofIn(const DomainShared_ptr& domain)
    {
        ...
        std::cout << "Condensed Dof of given domain are: "
                  << it->second-slave_it->second.size()-Dirichlet_it.size();
        std::cout << std::endl;
    }

    void IndicesToGlobal(const DomainShared_ptr& domain, std::vector<int>& indices) const
    {
        int start_index = StartingIndex(domain);
        std::transform(indices.cbegin(), indices.cend(), indices.begin(), [&start_index](const int& i)
        { return i+start_index; });
    }

    std::map<int, int> GlobalDirichletCondensedMap() const
    {
        std::map<int, int> res;
        int index = 0;
...
private:
    ...
    std::map<DomainShared_ptr, std::set<int>> _DirichletDof;
